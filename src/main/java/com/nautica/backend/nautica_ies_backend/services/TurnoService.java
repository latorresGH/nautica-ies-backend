...
public class TurnoService {
    ...
    public TurnoService(
            ...
            EmbarcacionRepository embarRepo,
            ClienteRepository clienteRepo,
            TurnoNotificationService turnoNotificationService,
            OperarioRepository operarioRepo
    ) {
    ...
    private void verificarSolapamiento(
        ...
        List<Turno> turnos = turnoRepo.findByFechaAndEmbarcacion_IdEmbarcacion(fecha, idEmbarcacion);
...
public TurnoCliente solicitarTurno(TurnoSolicitudRequest req) {
    ...
    // 🔔 Notificaciones push
    try {
        // 1) al cliente que pidió el turno
        turnoNotificationService.notificarTurnoCreadoCliente(guardado);

        // 2) a todos los operarios ACTIVOS
        //    Operario extiende Usuario, así que getIdUsuario() viene de la superclase
        var operariosActivos = operarioRepo.findByActivoTrue();

        var idsOperarios = operariosActivos.stream()
                .map(Operario::getIdUsuario)
                .collect(Collectors.toList());

        turnoNotificationService.notificarTurnoCreadoOperarios(guardado, idsOperarios);

    } catch (Exception e) {
        System.err.println("Error al enviar push de turno creado: " + e.getMessage());
    }
    ...
    private TipoTarea mapTipoTareaFromRequest(String raw) {
        ...
        switch (raw.toLowerCase()) {
            case "lavado":
                return TipoTarea.lavado;
            case "botado":
                return TipoTarea.botado;
            default:
    ...
    public TurnoCliente cancelarTurno(Long idTurno) {
        ...
        // 🔔 Notificación push al cliente por cancelación
        try {
            turnoNotificationService.notificarTurnoCanceladoCliente(turno);
        } catch (Exception e) {
            System.err.println("Error al enviar push de turno cancelado: " + e.getMessage());
        }
